...
//  Tier is the weighted average of the tiers of incoming demands, + 1,
//  or plus distance.  For primary consumers, tier = 0.

//  Urgency = shortage / ((stock + 5) * (2 + tier))

//  ...Only diffuse demand to nearby venues with lower urgency, and in
//  proportion to difference in urgency / distance.

//  ...Favour delivery to closer venues with higher urgency.
//  ...Favour collection from closer venues with lower urgency.



public class VenueStocks2 extends Inventory implements BuildConstants {
  ...
  public Batch <String> ordersDesc() {
    ...
    for (Demand demand : demands.values()) {
      final int needed = (int) demand.demandAmount ;
...
public void updateStocks(int numUpdates) {
  ...
  final Presences presences = venue.world().presences ;
  for (Demand d : demands.values()) {
    final float shortage = shortageOf(d.type) ;
    if (shortage <= 0) continue ;
    if (verbose && BaseUI.isPicked(venue)) I.say(
      venue+" demand for "+d.type+
      " amount/tier: "+d.demandAmount+"/"+d.demandTier
    ) ;
